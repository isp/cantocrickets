...
export default class Minting extends React.Component {
	...
	async startApp(provider) {
		...
		if (chainId != 0x1) {
			this.throwError("Please switch to Ethereum Mainnet.")
			return
		}
	...
	async mint() {
		...
		const totalPrice = web3.utils.toBN(tokenPrice).mul(amount > 1 && alreadyMinted == 0 && isWhitelisted ? amount - 1 : amount)
